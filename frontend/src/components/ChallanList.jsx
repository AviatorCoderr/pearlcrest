...
function ChallanList() {
  ...
  const generatePDF = (challan) => {
    const { _id, vehicle, amount, date } = challan;
    ...
    doc.setFont('helvetica', 'bold');
    doc.text(`${vehicle.reg_no}`, 50, 90);
  
    // Amount
    doc.setFont('helvetica', 'normal');
    doc.text('Amount:', 15, 100);
    doc.setFont('helvetica', 'bold');
    doc.text(`${amount}`, 50, 100);
  
    // Date
    doc.setFont('helvetica', 'normal');
    doc.text('Date:', 15, 110);
    doc.setFont('helvetica', 'bold');
    doc.text(`${new Date(date).toLocaleString('en-IN')}`, 50, 110);
  
    // Signature
    const signatureData = '/static/images/treasurersign.jpg';
    doc.addImage(signatureData, 'PNG', 80, 150, 40, 20);
  
    // Treasurer
    doc.setFontSize(12);
    doc.setFont('helvetica', 'bold');
    doc.text('Mr. Ravindra Prasad, President', 105, 175, null, null, 'center');
  
    // Dashed lines
    doc.setLineWidth(0.1);
    doc.setDrawColor(0);
    doc.setLineDashPattern([1, 1], 0);
    doc.line(0, 200, 400, 200); // Draw dashed line
  ...
  return (
    <div className="container mx-auto mt-10 p-4 bg-gray-100 rounded-lg shadow-lg">
      <h1 className="text-2xl font-semibold mb-4">Your Challans</h1>
